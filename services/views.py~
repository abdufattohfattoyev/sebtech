...
def get_dashboard_stats():
    ...
    try:
        ...
        # Xatolik bo‘lsa, default qiymatlar qaytariladi
        return {
            'active_services': 0,
...
def service_edit(request, pk):
    """Xizmatni tahrirlash - TELEFON ALMASHTIRILISHI MUMKIN"""
    try:
        service = get_object_or_404(MasterService, pk=pk)
        old_fee = service.service_fee
        old_phone = service.phone
            ...
            if form.is_valid():
                try:
                    with transaction.atomic():
                        ...
                        new_phone = new_service.phone

                        # TELEFON O'ZGARGANDA
                        if new_phone != old_phone:
                            # Eski telefonni do'konga qaytarish va repair_cost ni kamaytirish
                            old_phone.status = 'shop'
                            if old_phone.repair_cost >= old_fee:
                                old_phone.repair_cost -= old_fee
                            else:
                                old_phone.repair_cost = Decimal('0.00')
                            old_phone.save()

                            # Yangi telefonni ustaga yuborish va repair_cost ga qo'shish
                            new_phone.status = 'master'
                            new_phone.repair_cost = (new_phone.repair_cost or Decimal('0.00')) + new_service.service_fee
                            new_phone.save()
                        else:
                            # TELEFON O'ZGARMAGANDA - faqat fee o'zgarganda repair_cost yangilash
                            fee_difference = new_service.service_fee - old_fee
                            if fee_difference != 0:
                                new_phone.repair_cost = (new_phone.repair_cost or Decimal('0.00')) + fee_difference
                                new_phone.save(update_fields=['repair_cost'])

                        new_service.save()

                        # To'langan summani qayta hisoblash
                        new_service.update_paid_amount()

                    messages.success(request, 'Xizmat ma\'lumotlari yangilandi!')
                    return redirect('services:service_detail', pk=new_service.pk)
...
def service_delete(request, pk):
    """Xizmatni o'chirish - TO'G'RI ISHLAYDI"""
    try:
        service = get_object_or_404(MasterService, pk=pk)
        ...
        if request.method == 'POST':
            try:
                with transaction.atomic():
                    # Telefon va usta ma'lumotlarini saqlab qolish
                    phone = service.phone
                    service_fee = service.service_fee

                    # Telefonni do'konga qaytarish
                    phone.status = 'shop'

                    # Repair cost ni kamaytirish
                    if phone.repair_cost >= service_fee:
                        phone.repair_cost -= service_fee
                    else:
                        phone.repair_cost = Decimal('0.00')

                    phone.save()

                    # Qarzni o'chirish (model delete metodida bajariladi)
                    # Xizmatni o'chirish
                    service.delete()
        ...
        logger.error(f"Service delete view error: {str(e)}")
        messages.error(request, "Xizmat ma'lumotlarini yuklashda xatolik!")
        return redirect('services:service_list')
