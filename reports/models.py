...
class ReportCalculator:
    ...
    def get_seller_monthly_salary(self, seller, year, month):
        """Sotuvchi oylik maoshi - TARIX BILAN"""
        start_date = date(year, month, 1)
        _, last_day = monthrange(year, month)
        end_date = date(year, month, last_day)
        ...
        while current_date <= end_date:
            ...
            if day_report['counts']['total'] > 0 or day_report['counts']['returns'] > 0:
                # O'sha kun uchun foyda hisoblash
        ...
        # FOYDA - KUNLIK HISOBLASH
        net_phone_profit = Decimal('0')
        accessory_profit = Decimal('0')
        exchange_profit = Decimal('0')

        # Har bir sotuv o'z sanasi uchun hisoblanadi
        for sale in net_phone_sales:
            profit = self.profit_calc.calculate_phone_profit(sale)
            net_phone_profit += profit

        for sale in accessory_sales:
            profit = self.profit_calc.calculate_accessory_profit(sale)
            accessory_profit += profit

        for exchange in exchanges:
            profit = self.profit_calc.calculate_exchange_profit(exchange)
            exchange_profit += profit

        totals = self._calculate_sales_totals(phone_sales, accessory_sales, exchanges)
        returns_count = phone_returns.count()

        # KOMISSIYA - KUNLIK FOIZ BILAN
        ...
        while current_date <= end_date:
            ...
            # O'sha kunda sotilgan telefonlar
            ...
            # O'sha kunda sotilgan aksessuarlar
            ...
            # O'sha kunda bo'lgan almashtirishlar
        ...
        # Oxirgi kunda qanday bo'lsa, shuni ishlatish (maosh uchun)
