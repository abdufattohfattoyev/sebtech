...
def accessory_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                # Form o'zi hamma narsani hal qiladi
                accessory = form.save(commit=True)

                # Mavjud aksessuar yoki yangi?
                if hasattr(form, '_existing_accessory'):
                    quantity = form.cleaned_data.get('quantity', 0)
                    messages.success(
                        request,
                        f"✅ Mavjud aksessuar ustiga {quantity} dona qo'shildi!"
                    )
                else:
                    messages.success(
                        request,
                        f"✅ Yangi aksessuar '{accessory.name}' muvaffaqiyatli qo'shildi!"
                    )
...
def accessory_update(request, pk):
    """Aksessuarni tahrirlash - FAQAT boss/finance (SODDALASHTIRILGAN)"""
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                messages.success(
                    request,
                    f"✅ '{accessory.name}' ma'lumotlari yangilandi!"
                )
                return redirect('inventory:accessory_list')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                messages.success(
                    request,
                    f"✅ '{accessory.name}' ga {quantity} dona qo'shildi!"
                )
                return redirect('inventory:accessory_list')
