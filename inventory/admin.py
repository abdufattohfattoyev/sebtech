...
from .models import Supplier, ExternalSeller, PhoneModel, MemorySize, Accessory, Phone  # O'ZGARTIRILDI
...
@admin.register(ExternalSeller)  # O'ZGARTIRILDI
class ExternalSellerAdmin(admin.ModelAdmin):  # O'ZGARTIRILDI
    list_display = ('name', 'phone_number', 'phones_count', 'created_at', 'created_by')
    search_fields = ('name', 'phone_number')
    list_filter = ('created_at', 'created_by')
    readonly_fields = ('created_at',)

    fields = ('name', 'phone_number', 'image', 'notes', 'created_by', 'created_at')

    def phones_count(self, obj):
        count = obj.supplied_phones.count()  # O'ZGARTIRILDI: sold_phones -> supplied_phones
        return format_html('<span style="color: blue;">{}</span>', count)

    phones_count.short_description = "Yetkazib bergan telefonlar"  # O'ZGARTIRILDI
...
class PhoneAdmin(admin.ModelAdmin):
    ...
    def source_display(self, obj):
        if obj.source_type == 'supplier' and obj.supplier:
            return format_html('<span style="color: blue;">📦 Taminotchi: {}</span>', obj.supplier.name)
        elif obj.source_type == 'external_seller' and obj.external_seller:  # O'ZGARTIRILDI
            return format_html('<span style="color: green;">👤 Tashqi sotuvchi: {}</span>', obj.external_seller.name)
        elif obj.source_type == 'exchange':
            return format_html('<span style="color: purple;">🔄 Almashtirish</span>')
    ...
    def actual_profit_display(self, obj):
        """Haqiqiy foyda — faqat sotilgan telefonlar uchun haqiqiy sotish narxi bilan"""
        if obj.status == 'sold':
            try:
