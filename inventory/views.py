...
# inventory/views.py - dashboard funksiyasi (YANGILANGAN)
...
def dashboard(request):
    ...
    for shop in shops:
        # ✅ FAQAT DO'KONDAGI, USTADAGI VA QAYTARILGAN TELEFONLAR
...
def _ajax_phone_search(request, phones_queryset):
    """AJAX qidirish uchun yordamchi funksiya"""
    page = request.GET.get('page', 1)

    paginator = Paginator(phones_queryset, 10)
    try:
    ...
    results = {
        ...
        'stats': {
            'total_phones': all_phones.count(),
            'displayed': phones_page.paginator.count,
            'phones_in_shop': all_phones.filter(status='shop').count(),
            'phones_sold': all_phones.filter(status='sold').count(),
        }
    }
...
def phone_create(request):
    """Yangi telefon qo'shish - FAQAT boss/finance"""
    user_shops = Shop.objects.filter(owner=request.user)
    if not user_shops.exists():
        messages.error(request, "Sizda do'kon mavjud emas. Avval do'kon yarating.")
        return redirect('shop:dashboard')
...
def phone_update(request, pk):
    """Telefonni tahrirlash - FAQAT boss/finance"""
...
def phone_delete(request, pk):
    """Telefonni o'chirish - FAQAT boss/finance"""
    phone = get_object_or_404(Phone, pk=pk)
...
def accessory_list(request):
    ...
    # Har bir aksessuar uchun total_cost qo'shish
...
def accessory_create(request):
    """Yangi aksessuar qo'shish - FAQAT boss/finance"""
    user_shops = Shop.objects.filter(owner=request.user)
    if not user_shops.exists():
            ...
            try:
                ...
                if hasattr(form, '_existing_accessory'):
                    messages.success(request,
                                     f"Mavjud aksessuar ustiga {form.cleaned_data['quantity']} dona qo'shildi!")
                else:
                    messages.success(request, "Yangi aksessuar muvaffaqiyatli qo'shildi!")
...
def accessory_update(request, pk):
    """Aksessuarni tahrirlash - FAQAT boss/finance"""
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                form.save()
                messages.success(request, "Aksessuar ma'lumotlari yangilandi!")
                return redirect('inventory:accessory_list')
            except Exception as e:
                messages.error(request, f"Yangilashda xatolik: {str(e)}")
        else:
...
def accessory_delete(request, pk):
    """Aksessuarni o'chirish - FAQAT boss/finance"""
    accessory = get_object_or_404(Accessory, pk=pk)

    if request.method == 'POST':
        try:
            accessory.delete()
            messages.success(request, "Aksessuar muvaffaqiyatli o'chirildi!")
        except Exception as e:
            messages.error(request, f"O'chirishda xatolik: {str(e)}")
        return redirect('inventory:accessory_list')
...
def accessory_add_quantity(request, pk):
    """Mavjud aksessuar ustiga soni qo'shish - FAQAT boss/finance"""
    accessory = get_object_or_404(Accessory, pk=pk)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                messages.success(request, f'{accessory.name} ga {form.cleaned_data["quantity"]} dona qo\'shildi!')
                return redirect('inventory:accessory_list')
            except Exception as e:
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not purchase_price:
                return JsonResponse({
                    'success': False,
                    'message': "Tannarx kiritilishi kerak!"
                })
            ...
            accessory.save()  # quantity va purchase_price yangilanadi
...
def shop_detail(request, shop_id):
    """Do'kon tafsilotlari - faqat do'kondagi, ustadagi va qaytarilgan"""
...
def accessory_search(request):
    ...
    results = {
        'accessories': [{
            ...
            'total_cost': float(acc.purchase_price * acc.quantity),  # Har bir aksessuar uchun umumiy qiymat
...
def phone_detail(request, pk):
    ...
    if phone_sale:
        timeline.append({
            'date': phone_sale.sale_date,  # ⬅ faqat sale_date ishlatyapmiz
            'event': 'Sotildi',
    ...
    if phone_return:
        timeline.append({
            'date': getattr(phone_return, 'return_date', None),  # created_at o‘rniga return_date
            'event': 'Qaytarildi',
...
@login_required
def search_external_seller_api(request):  # YANGI: nomini o'zgartirdik
    ...
    sellers = ExternalSeller.objects.filter(  # O'ZGARTIRILDI
...
@login_required
def check_external_seller_phone_api(request):  # YANGI: nomini o'zgartirdik
    ...
    try:
        seller = ExternalSeller.objects.get(phone_number=phone)  # O'ZGARTIRILDI
        return JsonResponse({
            'exists': True,
            'seller': {
                'id': seller.id,
    ...
    except ExternalSeller.DoesNotExist:  # O'ZGARTIRILDI
        return JsonResponse({
            'exists': False,
