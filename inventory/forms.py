...
class AccessoryForm(forms.ModelForm):
    ...
    initial_purchase_price = forms.DecimalField(
        ...
        decimal_places=2,
        min_value=0,
        required=False,  # ✅ False - tahrirlashda kerak emas
    ...
    class Meta:
        model = Accessory
        fields = ['shop', 'name', 'code', 'image', 'sale_price', 'supplier']
        widgets = {
            'shop': forms.Select(attrs={'class': 'form-control'}),
            'name': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Aksessuar nomi'}),
            'code': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Kod (0001)'}),
            'image': forms.FileInput(attrs={'class': 'form-control'}),
            'sale_price': forms.NumberInput(attrs={
                'class': 'form-control',
                'step': '1000',
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk:
            # ✅ TAHRIRLASHDA: existing_code va initial_quantity ni o'chirmaymiz
            # Lekin ularni OPTIONAL qilamiz
            self.fields['existing_code'].widget = forms.HiddenInput()
            self.fields['initial_quantity'].required = False
            self.fields['initial_purchase_price'].required = False

            # ✅ Hozirgi qiymatlarni ko'rsatish uchun
            if hasattr(self.instance, 'purchase_price'):
                self.fields['initial_purchase_price'].initial = self.instance.purchase_price

        else:
            # Yangi yaratishda
            self.fields['code'].required = False
            self.fields['initial_quantity'].required = True
            self.fields['initial_purchase_price'].required = True
    ...
    def clean_initial_purchase_price(self):
        price = self.cleaned_data.get('initial_purchase_price')
        if price:
    ...
    def clean(self):
        ...
        quantity = cleaned_data.get('initial_quantity')
        purchase_price = cleaned_data.get('initial_purchase_price')
        sale_price = cleaned_data.get('sale_price')

        # ✅ TAHRIRLASHDA
        if self.instance.pk:
            # Faqat sale_price validatsiyasi
        ...
        if not quantity or quantity <= 0:
            raise ValidationError({'initial_quantity': "Soni kiritilishi kerak va 0 dan katta bo'lishi kerak."})

        if not purchase_price or purchase_price <= 0:
            raise ValidationError(
                {'initial_purchase_price': "Tannarx kiritilishi kerak va 0 dan katta bo'lishi kerak."})
    ...
    def save(self, commit=True):
        quantity = self.cleaned_data.get('initial_quantity', 0)
        purchase_price = self.cleaned_data.get('initial_purchase_price', Decimal('0'))

        # ✅ TAHRIRLASH
        if self.instance.pk:
            accessory = super().save(commit=False)

            # Sale price allaqachon accessory.sale_price da
            # Agar yangi quantity va purchase_price berilgan bo'lsa
            if quantity and quantity > 0 and purchase_price and purchase_price > 0:
                if commit:
                    # Avval accessory ni saqlash
                    accessory.save()

                    # Keyin purchase history yaratish
                ...
                # Faqat sale_price o'zgargan bo'lsa
                accessory.save()
