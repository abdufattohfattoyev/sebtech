...
class PhoneSale(models.Model):
    ...
    phone = models.OneToOneField(Phone, on_delete=models.CASCADE, related_name="phonesale", verbose_name="Telefon")
    ...
    def clean(self):
        """Validatsiya"""
        # ID orqali tekshirish - RelatedObjectDoesNotExist xatosini oldini olish
        ...
        # Faqat do'kon va qaytarilgan telefonlar - phone_id orqali tekshirish
    ...
    def delete(self, *args, **kwargs):
        ...
        # Telefon statusini tiklash
        if self.phone_id:
            Phone.objects.filter(id=self.phone_id).update(status='shop')

        # Qarzlarni o'chirish (IMEI bo'yicha qidirish)
        if self.phone_id and self.customer_id:
            try:
                phone = Phone.objects.get(id=self.phone_id)

                # Mijoz → Sotuvchi qarzi (IMEI bo'yicha)
                Debt.objects.filter(
                ...
                # Sotuvchi → Boss qarzi (IMEI bo'yicha)
                if self.salesman_id:
                    Debt.objects.filter(
...
class PhoneReturn(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.phone_sale_id and self.phone_sale.phone_id:
            Phone.objects.filter(id=self.phone_sale.phone_id).update(status='returned')
