...
class DebtForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if debt_amount:
            if currency == 'USD' and debt_amount > 3000:
...
class ExpenseForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # ✅ BARCHA FOYDALANUVCHILAR UCHUN BARCHA DO'KONLAR
        self.fields['shop'].required = True
        shops = Shop.objects.all().order_by('name')
        self.fields['shop'].queryset = shops

        if shops.exists():
            # ✅ Choices ni to'g'ri belgilash
            self.fields['shop'].empty_label = "Do'konni tanlang"
        else:
            self.fields['shop'].empty_label = "Do'konlar mavjud emas"
            self.fields['shop'].widget.attrs['disabled'] = True
