...
class PhoneSaleAdmin(admin.ModelAdmin):
    ...
    date_hierarchy = 'sale_date'  # ✅ TO'G'RI — PhoneSale.sale_date mavjud
    ...
    def delete_model(self, request, obj):
        """Bitta obyektni o'chirish — PhoneSale.delete() metodini chaqiradi"""
        ...
        try:
            # PhoneSale.delete() metodini chaqirish — u barcha bog'liq qarz va telefon holatini tozalaydi
            obj.delete()
            messages.success(
                request,
                f"✅ Telefon sotish muvaffaqiyatli o'chirildi:\n"
                f"📱 Telefon: {phone_info} - do'konga qaytarildi\n"
                f"👤 Mijoz: {customer_info}{debt_info}\n"
                f"🗑️ Barcha bog'liq qarzlar va ma'lumotlar tozalandi"
            )
        except Exception as e:
            messages.error(request, f"❌ O'chirishda xatolik: {str(e)}")

    def delete_queryset(self, request, queryset):
        """Bir nechta obyektni o'chirish — har biri uchun delete_model chaqiriladi"""
        ...
        # Har birini alohida o'chiramiz — chunki custom delete() logikasi bor
        for obj in queryset:
            try:
                obj.delete()
                success_count += 1
        ...
        if errors:
            ...
            message = f"✅ {success_count} ta telefon sotish muvaffaqiyatli o'chirildi"
            if debt_total > 0:
                message += f"\n💰 Tozalangan qarzlar: ${debt_total}"
            message += f"\n📱 Barcha telefonlar do'konga qaytarildi"
    ...
    def payment_summary(self, obj):
        ...
        return format_html(" | ".join(payments)) if payments else "—"
...
class AccessorySaleAdmin(admin.ModelAdmin):
    ...
    def delete_model(self, request, obj):
        ...
        try:
            ...
            messages.success(
                ...
                f"🔧 Aksessuar: {accessory_info} - zahiraga qaytarildi\n"
                f"👤 Mijoz: {customer_info}{debt_info}\n"
                f"🗑️ Barcha bog'liq ma'lumotlar tozalandi"
            )
        except Exception as e:
            messages.error(request, f"O'chirishda xatolik: {str(e)}")
    ...
    def delete_queryset(self, request, queryset):
        ...
        try:
            ...
            message += f"\n📦 Jami {quantity_total} dona aksessuar zahiraga qaytarildi"
            if debt_total > 0:
                message += f"\n💰 Tozalangan qarzlar: ${debt_total}"
    ...
    def payment_summary(self, obj):
        payments = []
        if obj.cash_amount > 0:
            payments.append(format_html('<span style="color: green;">Naqd: {:,} soʻm</span>', int(obj.cash_amount)))
        if obj.card_amount > 0:
            payments.append(format_html('<span style="color: blue;">Karta: {:,} soʻm</span>', int(obj.card_amount)))
        if obj.credit_amount > 0:
            payments.append(
                format_html('<span style="color: purple;">Nasiya: {:,} soʻm</span>', int(obj.credit_amount)))
        if obj.debt_amount > 0:
            payments.append(format_html('<span style="color: red;">Qarz: {:,} soʻm</span>', int(obj.debt_amount)))
        return " | ".join(payments) if payments else "Toʻlov yoʻq"

    payment_summary.short_description = "Toʻlovlar"
    ...
    def profit_display(self, obj):
        try:
            ...
            color = "green" if profit >= 0 else "red"
            return format_html('<span style="color: {};">{:,} soʻm</span>', color, int(profit))
        except:
            return "N/A"
...
class PhoneExchangeAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Asosiy ma'lumotlar", {
            'fields': (
                ...
                'exchange_type',
                'notes',
                # ✅ 'created_by' — OLIB TASHLANDI — modelda yo'q
            )
        }),
        ("Yangi Telefon", {
            'fields': ('new_phone', 'new_phone_price')
        }),
        ("Eski Telefon — Batafsil Ma'lumotlar", {
            'fields': (
                'old_phone_model',
                'old_phone_memory',
                'old_phone_imei',
    ...
    def old_phone_display(self, obj):
        return f"{obj.old_phone_model} {obj.old_phone_memory} — {obj.old_phone_evaluated_price}$"

    old_phone_display.short_description = "Eski Telefon"

    def new_phone_display(self, obj):
        if obj.new_phone:
            return f"{obj.new_phone.phone_model} {obj.new_phone.memory_size} — {obj.new_phone.sale_price}$"
    ...
    def save_model(self, request, obj, form, change):
        ...
        # Lekin modelda bu maydon yo'q — shu sababli o'chirildi
...
class PhoneReturnAdmin(admin.ModelAdmin):
    ...
    def phone_display(self, obj):
        ...
        if obj.phone_sale and obj.phone_sale.phone:
            ...
            return f"{phone.phone_model} {phone.memory_size} — IMEI: {phone.imei or 'N/A'}"
        return "Telefon yo'q"
